...
def test_quote_consistency():
    assert "change_pct_inconsistent" not in validate_quote_fields(
        {"price": 57.21, "prev_close": 55.63, "change_pct": 2.84}
    )
...
def test_trade_date_uses_valid_modal_date_across_sources():
    ...
    tencent = {
        "600000": {"date": "2026-08-12"},
        "000001": {"date": "2026-08-12"},
    }
...
def test_exchange_aware_filter_blocks_index_collisions():
    assert is_main_board_symbol("sz000001")
    assert is_main_board_symbol("sh600000")
    assert not is_main_board_symbol("sh000001")
    assert not is_main_board_symbol("sh000971")
    assert not is_main_board_symbol("sz399001")
