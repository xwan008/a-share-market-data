...
## 2. 扫描节奏：周度全量 + 日度增量
### 2.1 周度全量盈利普查
扫描基本单元是申万2021 **346个三级节点**。正式全量锚点固定为**每周五18:00（Asia/Shanghai）**；成功bootstrap后，下一次全量就是最近一个周五18:00，即使距离bootstrap不足7天。7天只表示周度基线的最大有效年龄，不是必须等待满7天。若周五全量失败或未运行，则下一次可用工作日18:00补做。若当前schema没有有效周度基线，则下一次18:00无论星期几都必须先做bootstrap全量普查。
