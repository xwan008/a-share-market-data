...
## 目标
V2从正在改善的细分盈利驱动中，找到未来1–2季度仍有兑现逻辑、但股价尚未充分定价的公司，并最终形成可解释的**基本面公允价值锚、安全买入区、合理买入区、历史参照和价格状态**。
...
6. 最好的机会通常是“基本面已拐、价格刚启势”，而不是极端便宜或已经大涨。
...
V2必须自己重建，不读取V1估值数值。固定顺序是：`真实盈利 → 盈利类型 → Forward/正常化盈利 → 合理估值倍数 → Fair Value`。

- **成长/稳定经营**：Forward EPS × 合理PE。合理PE必须受未来1–2年增长持续性与盈利质量约束，PEG用于sanity；高增长不能仅靠远期故事抬升买点，次年盈利转弱必须压制低风险倍数。
...
正式安全/合理区由**A + Margin of Safety**形成：
- 有显式`low_risk_multiple_range`时直接使用entry multiple，禁止再次统一折价；
- 无显式entry multiple时使用版本化`safe_to_fair_floor / reasonable_to_fair_floor`从Fair Value形成区间；安全边际反映盈利持续性和估值不确定性；
...
每只候选固定输出：股票/代码/当前价、盈利Driver、Forward Bridge、基本面锚、独立确认锚、历史参照、安全区、合理区、当前价值位置、价格结构、追高风险、动作、失效条件和blocker。

优先寻找：`盈利改善明确 + Forward Bridge成立 + 估值模型匹配 + 正式买入区成立 + 当前真的在区间内 + 价格刚启势/低风险回踩`。
