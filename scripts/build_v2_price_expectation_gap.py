...
def value_state(price,row):
    ...
    if price <= safe[1]: return 'safe_buy_zone',gap
    if price <= reasonable[1]: return 'reasonable_buy_zone',gap
...
def combine(vs,structure):
    ...
    if vs=='deep_discount_review': return 'fundamental_price_conflict' if structure=='damaged' else 'deep_value_review'
