...
def atr_pct(rows: list[dict], n: int = 14):
    ...
    for i in range(len(rows)-n, len(rows)):
        prev = rows[i-1]["close"]
        trs.append(max(rows[i]["high"]-rows[i]["low"], abs(rows[i]["high"]-prev), abs(rows[i]["low"]-prev)))
    return (sum(trs)/len(trs))/rows[-1]["close"]


def volume_metrics(rows: list[dict]):
    prev20 = [r["volume"] for r in rows[-21:-1] if isinstance(r.get("volume"), (int, float)) and r["volume"] > 0]
    recent5 = [r["volume"] for r in rows[-5:] if isinstance(r.get("volume"), (int, float)) and r["volume"] > 0]
...
def base_metrics(code: str, name: str, rows: list[dict], quote: dict, reference_date: str):
    ...
    base_flags = {"risk_warning": flagged, "risk_warning_reason": flag_reason, "low_risk_eligible": not flagged}
    if len(rows) < MIN_POINTS:
        return {"code": code, "name": name, **base_flags, "data_status": "unavailable", "reason": f"history_insufficient:{len(rows)}"}
    if rows[-1]["date"] != reference_date:
        return {"code": code, "name": name, **base_flags, "data_status": "unavailable", "reason": f"history_stale:{rows[-1]['date']}!={reference_date}"}
    ...
    if ma20 < current: supports.append(ma20)
    if ma60 < current: supports.append(ma60)
    support = max(supports) if supports else min(r["low"] for r in rows[-20:])

    return {
        "code": code, "name": name, **base_flags, "data_status": "verified", "data_date": rows[-1]["date"], "history_points": len(rows),
        "current_price": round(current, 4), "ma20": round(ma20, 4), "ma60": round(ma60, 4),
        "ma20_slope_5d_pct": round(slope20*100, 2), "ma60_slope_5d_pct": round(slope60*100, 2),
        "return_10d_pct": round(ret10*100, 2), "return_20d_pct": round(ret20*100, 2),
        "distance_to_ma20_pct": round(dist20*100, 2), "distance_to_ma60_pct": round(dist60*100, 2),
        "atr14_pct": round((atr_pct(rows) or 0)*100, 2), "prior_60d_high": round(prev60_high, 4), "prior_120d_high": round(prev120_high, 4),
        "higher_high": hh, "higher_low": hl, "first_effective_resistance": resistance, "support_invalidation": round(support, 4),
        ...
        "close_location_pct": round(close_loc*100, 2), "current_day_high": rows[-1]["high"], "raw_return_20d": ret20,
    }
...
def classify(row: dict, market_ret20: float):
    ...
    chase = "high" if d20 > 0.12 or r10 > 0.18 or (atr > 0.06 and r10 > 0.12) else ("medium" if d20 > 0.07 or r10 > 0.12 or d60 > 0.18 else "low")
    above = p > ma20 > ma60
    ...
    volume_confirmed = bool((row.get("volume_ratio_1d_vs_20d") or 0) >= 1.15 or (row.get("volume_ratio_5d_vs_20d") or 0) >= 1.05)
    ...
    row["price_discovery"] = bool(breakout_confirmed and p >= row["prior_120d_high"] * 0.995 and row.get("first_effective_resistance") is None)
    row["resistance_context"] = "price_discovery_or_new_high; absence of overhead resistance is not a negative signal" if row["price_discovery"] else ("below_known_resistance" if row.get("first_effective_resistance") else "no_nearby_mapped_resistance")
    ...
    row["downside_to_invalidation_pct"] = round((p/support - 1)*100, 2) if support and support < p else None
    res = row.get("first_effective_resistance")
    row["upside_to_first_resistance_pct"] = round((res["price"]/p - 1)*100, 2) if isinstance(res, dict) and f(res.get("price")) and res["price"] > p else None
    return row
...
def main():
    ...
    for code, quote in stocks.items():
        ...
        row = base_metrics(code, (quote or {}).get("name") or code, rows_for(code), quote or {}, reference_date)
        base[code] = row
    ...
    candidates = sorted(code for code, row in results.items() if row.get("low_risk_eligible") and row.get("structure_type") in {"trend_continuation", "breakout", "pullback"} and row.get("chase_risk") != "high")
    ...
    payload = {
        "schema_version": 2, "mode": "shadow", "generated_at": datetime.now(TZ).isoformat(), "reference_trade_date": reference_date,
        "universe_source": "all_mainboard_codes_from_data/latest.json", "universe_count": len(stocks), "verified_count": len(stocks)-len(unavailable),
        "unavailable_count": len(unavailable), "risk_warning_scanned_count": risk_count, "market_median_return_20d_pct": round(market_ret20*100, 2),
        "structure_type_counts": counts, "right_candidate_codes": candidates, "unavailable_codes": unavailable, "companies": results,
        "method_note": "Full mainboard scan remains independent from fundamentals. Low-risk eligibility excludes risk-warning names only after scanning. Breakouts require price, volume and close confirmation; pullbacks require relative strength not materially below market."
    }
    OUT.parent.mkdir(parents=True, exist_ok=True)
    OUT.write_text(json.dumps(payload, ensure_ascii=False, indent=2), encoding="utf-8")
    print(json.dumps({"status":"ok","universe":len(stocks),"verified":payload["verified_count"],"risk_warning_scanned":risk_count,"right_candidates":len(candidates),"types":counts}, ensure_ascii=False))
