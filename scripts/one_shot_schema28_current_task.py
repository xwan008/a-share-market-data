...
HEAD={'User-Agent':'Mozilla/5.0 schema28-current-task'}
...
idx=json.loads(INDEX.read_text(encoding='utf-8'))['companies']
...
# Use only the validated industry baseline + current mapping to reconstruct company discovery.
l3={x['code']:x for x in s['coverage_ledger']['level3']}
confirmed=[]
for ch in s.get('profit_chains',[]):
    codes=[c for c in (ch.get('source_coverage_codes') or []) if c in l3]
    if ch.get('resolution_status')=='resolved' and any(l3[c].get('trend')=='improving' for c in codes): confirmed.append(ch)
by_l3=defaultdict(list)
for code,m in idx.items():
    if m.get('mapping_status')=='mapped' and m.get('sw_level3_code'): by_l3[m['sw_level3_code']].append((code,m))

light={}; comparisons=[]; companies={}; total=excluded=survivor_total=0; all_surv=set()
for ch in confirmed:
    cid=ch['chain_id']; source=[c for c in (ch.get('source_coverage_codes') or []) if c in l3]
    universe=[]
    for l3c in source: universe.extend(by_l3.get(l3c,[]))
    ded={c:m for c,m in universe}; screened=[]; surv=[]; scored=[]
    for c,m in sorted(ded.items()):
        total+=1; cr=cur.get(c); pr=prev.get(c)
        ce=num((cr or {}).get('DEDUCT_BASIC_EPS')); pe=num((pr or {}).get('DEDUCT_BASIC_EPS')); cy=yoy(ce,pe)
        rv=yoy(num((cr or {}).get('TOTAL_OPERATE_INCOME')),num((pr or {}).get('TOTAL_OPERATE_INCOME')))
        sh=shares_implied(cr or {}); parent=num((cr or {}).get('PARENT_NETPROFIT')); cp=core_profit(cr or {},sh)
        ratio=(abs(parent-cp)/abs(parent)) if parent and cp is not None else None
        ...
        if decision=='exclude': excluded+=1
            ...
            surv.append(c); all_surv.add(c); survivor_total+=1
            core_scale=abs(cp or 0); score=max(-1,min(3,cy or 0))*.55+max(-1,min(1,rv or 0))*.2+(math.log10(max(core_scale,1))/12)*.25
            scored.append((score,c))
            companies[c]={'code':c,'name':m.get('name') or (cr or {}).get('SECURITY_NAME_ABBR') or c,'sw_level1_code':m.get('sw_level1_code'),'sw_level1_name':m.get('sw_level1_name'),'sw_level2_code':m.get('sw_level2_code'),'sw_level2_name':m.get('sw_level2_name'),'sw_level3_code':m.get('sw_level3_code'),'sw_level3_name':m.get('sw_level3_name'),'source_chain_ids':sorted(set((companies.get(c) or {}).get('source_chain_ids',[])+[cid])),'core_h1_deduct_eps':ce,'core_h1_yoy':cy,'comparison_score':round(score,6)}
        screened.append({'code':c,'name':m.get('name') or c,'source_chain_ids':[cid],'business_exposure_match':True,'profit_driver_match':decision=='survive','earnings_quality_match':decision=='survive','comparability':decision=='survive','screen_decision':decision,'exclusion_reason':reason,'evidence_basis':f"SW2021 L3={m.get('sw_level3_name')}; deduct EPS {pe}->{ce}; yoy={cy}; revenue_yoy={rv}; nonrec_ratio={ratio}",'core_earnings_evidence':f"2026H1 deduct_basic_eps={ce}; 2025H1 deduct_basic_eps={pe}",'core_earnings_trend':'improving' if decision=='survive' else ('unconfirmed' if ce is None or pe is None else 'deteriorating'),'nonrecurring_dominance_check':{'dominant':bool(ratio is not None and ratio>=.30),'ratio_approx':ratio,'parent_netprofit':parent,'core_profit_implied':cp},'valuation_earnings_basis_requirement':'deducted_or_core_earnings_only'})
    scored.sort(reverse=True)
    light[cid]={'chain_id':cid,'screened_companies':screened,'screen_complete':True}
    comparisons.append({'chain_id':cid,'screened_companies':[x['code'] for x in screened],'excluded_companies':[x['code'] for x in screened if x['screen_decision']=='exclude'],'compared_companies':sorted(surv),'comparison_complete':True,'fundamental_best':scored[0][1] if scored else None,'current_opportunity_best':None,'opportunity_resolution_complete':False,'singleton_reason':'only_one_survivor_after_core_earnings_screen' if len(surv)==1 else None})
...
for c in valuation_set:
    ...
    elif cls=='financial': blocked=True;rc='critical_public_data_unavailable';blocker='金融公司需PB-ROE独立模型，当前通用PE模型不替代'
    ...
    if not blocked:
        ...
        bps[c]={'code':c,'value_eligible':value,'timing_eligible':timing,'buy_point_status':status,'buy_price_range':inter,'buy_point_basis':'safe_price_range ∩ independent structure_entry_range; current price must be inside','structure_entry_range':er,'invalidation_price':ps.get('support_invalidation')}
...
for comp in comparisons:
    comp['current_opportunity_best']=next((c for c in comp['compared_companies'] if c in buycodes),None); comp['opportunity_resolution_complete']=True
all_nonreview={c for c,v in vals.items() if not v['review_required']}
gate=(len(confirmed)==len(light) and set(bps)==all_nonreview and len(vals)==len(valuation_set) and all(x.get('screen_complete') for x in light.values()))

s.update({'generated_at':datetime.now(TZ).isoformat(),'run_type':'manual_schema28_current_task_from_valid_industry_baseline','status':'research_complete' if gate else 'incomplete_research','scan_mode':'baseline_reuse_company_refresh','company_light_screen':light,'chain_comparisons':comparisons,'companies':companies,'valuation_set':valuation_set,'valuations':vals,'price_structures':pstruct,'buy_point_assessments':bps,'current_opportunities':ops if gate else []})
s['diagnostics']['company_screen']={'confirmed_improving_chain_count':len(confirmed),'company_screened_chain_count':len(light),'unscreened_confirmed_improving_chains':[] if len(confirmed)==len(light) else [x['chain_id'] for x in confirmed if x['chain_id'] not in light],'light_screen_universe_company_count':total,'light_screen_excluded_count':excluded,'horizontal_comparison_survivor_count':survivor_total,'deduplicated_valuation_set_count':len(valuation_set)}
s['diagnostics']['valuation']={'valuation_set_count':len(valuation_set),'executed_count':len(vals),'complete_non_review_count':len(all_nonreview),'review_required_count':review,'extreme_deviation_audit_count':extreme,'valuation_gate_passed':gate}
...
print(json.dumps({'status':s['status'],'confirmed_chains':len(confirmed),'screened_company_instances':total,'excluded':excluded,'survivors_before_dedup':survivor_total,'valuation_set':len(valuation_set),'review_required':review,'extreme_audits':extreme,'buyable_now':len(ops),'opportunities':ops},ensure_ascii=False,indent=2))
