...
def fetch_anchor(ak, symbol: str, reference_trade_date: str) -> dict:
    ...
    if len(pairs) < 60:
        raise RuntimeError(f'insufficient futures history:{symbol}:{len(pairs)}')
    last_date = pairs[-1][0]
    ...
    high60 = max(vals[-60:])
    return {
        'symbol': symbol, 'last_date': last_date, 'age_days': age_days,
        'current': current, 'ma20': ma20, 'ma60': ma60,
        'current_to_ma20': current / ma20, 'current_to_ma60': current / ma60,
...
def main() -> int:
    ...
    for code in common.get('common_pool_codes', []):
        ...
        for a in policy.get('anchors', []):
            ...
            weighted_delta += weight * (m['current_to_ma60'] - 1.0) * direction
        ...
        if missing or consensus_missing or price is None:
            ...
            companies.append({
                'code': code, 'name': name, 'cycle_tag': matched_tag, 'current_price': price,
                'valuation_status': 'unavailable', 'valuation_model': 'forward_cycle_scenario',
                'execution_state': 'input_data_insufficient',
                'forward_earnings_basis': f'{year}/{year+1} consensus plus commodity and 6-18m regime are mandatory for resource valuation.',
        ...
        if eps_next is None:
            # Non-resource cycle policies can remain current-year based until a dedicated spread model is added.
            eps_next = eps_now
        ...
        short_term_effect = max(-max_short_effect, min(max_short_effect, weighted_delta * sensitivity))
        ...
        scenario_eps = [max(0.01, forward_eps * float(f) * (1.0 + short_term_effect)) for f in regime_factors]
        bear_eps, base_eps, bull_eps = scenario_eps
        mult_lo, mult_hi = float(multiple[0]), float(multiple[1])
        fair_floor = base_eps * mult_lo
        value_range = [bear_eps * mult_lo, bull_eps * mult_hi]
        ...
        row = {
            ...
            'valuation_status': 'valid', 'execution_state': 'valid', 'valuation_model': 'forward_cycle_scenario',
            ...
            'forward_earnings_basis': f'{year}/{year+1} consensus -> calendar-weighted forward-12m EPS {forward_eps:.4f} -> structured 6-18m regime -> capped short-term commodity modifier {short_term_effect:+.2%}.',
            'reasonable_multiple_range': [mult_lo, mult_hi],
            'multiple_rationale': regime.get('summary') if regime else policy.get('rationale') or 'Versioned cycle multiple after forward earnings normalization.',
            ...
            'key_sensitivities': ['6-18m supply-demand regime', 'next-year consensus EPS', 'commodity/cost anchor', 'production/sales'],
    ...
    payload = {
        ...
        'method_note': 'Resource cycle valuation uses current+next-year consensus to build a forward-12m earnings proxy, then applies a reviewed 6-18m supply-demand regime. Current/MA60 futures strength is capped as a near-term modifier and cannot override medium-term normalization risk.',
    }
